...
| Audit vendor-owned citations for commercial queries such as `best`, `alternative`, `comparison`, or `vs` | Load `references/playbooks/maintenance/vendor-owned-commercial-query-citations.md`, copy `assets/vendor-owned-commercial-citation-audit.csv`, then run `workflows/category-citation-loop.md` and `workflows/geo-audit.md`; route only justified page briefs to `workflows/content-production.md` and keep publication or outreach approval-gated |
...
- **Vendor-owned commercial-query citations** are scoped observations, not a page-count or formatting formula. Record source ownership, provider/surface/query/date/locale/account state, vendor disclosure, checked product truth, fair criteria, and material limitations; keep Search, Google AI impressions, provider citations, referrals, conversions, and revenue separate.
