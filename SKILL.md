...
| Turn verified customer questions into a bounded search-led founder or expert answer series | Load `references/playbooks/content/cross-platform-commercial-intent-distribution.md`, copy `assets/search-led-expert-answer-map.csv`, then run `workflows/content-production.md`; pilot 3–10 questions, keep owned/search/social/answer-provider outcomes separate, and require explicit approval before every external publication |
...
| Reconcile Organization/entity identity and audit `sameAs` candidates without creating profiles or promising rankings | Load `references/playbooks/content/organization-entity-reconciliation.md`, copy `assets/organization-entity-identity-audit.csv`, then run `workflows/site-audit.md` or `workflows/technical-seo-maintenance.md`; include only URLs that unambiguously identify the exact schema subject |
...
- **Organization `sameAs` reconciliation** is identity-data hygiene, not link building or a ranking shortcut. Use one stable entity node, verify that every candidate URL unambiguously identifies the exact schema subject, separate organizations, people, products, repositories, reviews, and mentions, and prefer a short accurate list over bulk profile URLs. External profile creation, claiming, or repair requires separate exact approval. Validation does not prove ranking lift, a Knowledge Panel, backlink value, or AI citations.
...
- **Hosted-publishing/API tactics** start with one useful, original, site-specific pilot. Telegra.ph API submission is
  ...
  and outcomes. Reject duplicated, spun, thin, doorway, exact-match-anchor, or unlimited cross-site publishing. Account
  and page writes require explicit approval for the exact article and destination.
