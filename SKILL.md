...
| Audit schema markup and rich-result eligibility without promising rankings or AI citations | Load `references/playbooks/content/schema-types-reference.md`, copy `assets/structured-data-verification-ledger.csv`, then run `workflows/site-audit.md` or `workflows/technical-seo-maintenance.md`; keep validators, Search appearance, Search performance, named AI-provider observations, referrals, conversions, and revenue separate |
...
- **Schema verification** starts with the real page purpose and current Google feature documentation, not a generator or “entity optimized” label. Treat generated markup as a draft, require visible-content parity plus separate Schema.org and Google rich-result checks, verify the live deployment, and pilot a representative cohort before template-wide rollout. Valid syntax or eligibility does not prove rankings, rich-result display, AI citations, traffic, conversions, or revenue.
