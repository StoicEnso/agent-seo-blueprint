...
class CrossPlatformCitationLoopDocsTest(unittest.TestCase):
    ...
    def test_readme_counts_match_repo(self):
        ...
        self.assertEqual(len(PLAYBOOKS), 65)
        self.assertEqual(len(WORKFLOWS), 11)
        self.assertIn("## The knowledge layer (65 playbooks)", readme)
        self.assertIn("#the-knowledge-layer-65-playbooks", readme)
        self.assertIn("**65 playbooks**", readme)
        self.assertIn("36 original course distillations plus 29 operational additions", readme)
        self.assertIn("36 course distillations + 29 operational additions", readme)
        self.assertIn("content/         (18)", readme)
        self.assertIn("cross-platform-commercial-intent-distribution", readme)
        self.assertIn("authority/       (17)", readme)
