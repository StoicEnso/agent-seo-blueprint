...
class CrossPlatformCitationLoopDocsTest(unittest.TestCase):
    ...
    def test_readme_counts_match_repo(self):
        ...
        self.assertEqual(len(PLAYBOOKS), 68)
        self.assertEqual(len(WORKFLOWS), 11)
        self.assertIn("## The knowledge layer (68 playbooks)", readme)
        self.assertIn("#the-knowledge-layer-68-playbooks", readme)
        self.assertIn("**68 playbooks**", readme)
        self.assertIn("36 original course distillations plus 32 operational additions", readme)
        self.assertIn("36 course distillations + 32 operational additions", readme)
        self.assertIn("content/         (18)", readme)
        self.assertIn("cross-platform-commercial-intent-distribution", readme)
        self.assertIn("authority/       (19)", readme)
        self.assertIn("government-supplier-registry-profiles", readme)
        self.assertIn("foundations/     (6)", readme)
        self.assertIn("maintenance/     (14)", readme)
