...
class CrossPlatformCitationLoopDocsTest(unittest.TestCase):
    ...
    def test_readme_counts_match_repo(self):
        ...
        self.assertEqual(len(PLAYBOOKS), 62)
        self.assertEqual(len(WORKFLOWS), 10)
        self.assertIn("## The knowledge layer (62 playbooks)", readme)
        self.assertIn("#the-knowledge-layer-62-playbooks", readme)
        self.assertIn("**62 playbooks**", readme)
        self.assertIn("36 original course distillations plus 26 operational additions", readme)
        self.assertIn("36 course distillations + 26 operational additions", readme)
