...
class CrossPlatformCitationLoopDocsTest(unittest.TestCase):
    ...
    def test_readme_counts_match_repo(self):
        ...
        self.assertEqual(len(PLAYBOOKS), 56)
        self.assertEqual(len(WORKFLOWS), 8)
        self.assertIn("## The knowledge layer (56 playbooks)", readme)
        self.assertIn("#the-knowledge-layer-56-playbooks", readme)
        self.assertIn("**56 playbooks**", readme)
        self.assertIn("36 original course distillations plus 20 operational additions", readme)
        self.assertIn("36 course distillations + 20 operational additions", readme)
