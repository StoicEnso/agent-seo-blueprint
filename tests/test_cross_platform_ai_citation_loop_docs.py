...
class CrossPlatformCitationLoopDocsTest(unittest.TestCase):
    ...
    def test_readme_counts_match_repo(self):
        ...
        self.assertEqual(len(PLAYBOOKS), 67)
        self.assertEqual(len(WORKFLOWS), 11)
        self.assertIn("## The knowledge layer (67 playbooks)", readme)
        self.assertIn("#the-knowledge-layer-67-playbooks", readme)
        self.assertIn("**67 playbooks**", readme)
        self.assertIn("36 original course distillations plus 31 operational additions", readme)
        self.assertIn("36 course distillations + 31 operational additions", readme)
