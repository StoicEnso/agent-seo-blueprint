...
4. **On-page audit.** Per priority page, scrape it with `serp_capture.py` and check against `on-page-optimization.md`: meta title contains the main keyword AND matches search intent (no obscure titles that cause bounces); meta description uses variants; internal links present and pointing at related pages; H1/H2 structure sane. When the page or SERP is meaningfully visual, load `image-search-optimization.md` and sample priority images for HTML discovery, contextual/accessible alt behavior, stable URLs, responsive sizing, transfer cost/CWV impact, image-sitemap need, and truthful conditional metadata. Record each gap as a finding; do not infer image indexation or traffic from markup alone.
   ...
   When structured data is in scope, load `schema-types-reference.md` and copy `assets/structured-data-verification-ledger.csv` to the workspace. Sample each material template. Record source/rendered delivery, initial-HTML posture, visible-content parity, current Google feature support, required/recommended property completeness, Rich Results Test status, Schema.org vocabulary status, and live Search Console evidence where available. Keep validator output, Search appearance, ordinary Search performance, named AI-provider observations, referrals, conversions, and revenue separate. The audit stays read-only and does not treat clean syntax as ranking, rich-result, or AI-citation proof.
...
- `audits/<date>_structured-data-verification-ledger.csv` — optional template-level schema, validation, deployment, and measurement evidence.
