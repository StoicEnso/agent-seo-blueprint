...
2. **Pick tactics for this project.** Choose from the menu based on fit (you can run several):
   ...
   - **Press-release distribution** (`press-release-distribution.md`) — only for a real, timely company event with an owned evidence hub. Verify destination-owned terms, a fresh public sample, link qualification, moderation, rights, disclosure, correction cost, and publication timing. Draft and hold the exact release; never repeat creator claims about followed links, minute-level indexing, Google News, ranking, traffic, or citations as guarantees.
