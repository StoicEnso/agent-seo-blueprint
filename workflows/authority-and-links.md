...
playbooks:
  ...
  - references/playbooks/authority/government-supplier-registry-profiles.md
  - references/playbooks/authority/hosted-publishing-experiments.md
  - references/playbooks/authority/press-release-distribution.md
...
outputs:
  ...
  - outreach/<date>_government-supplier-registries.csv # eligibility + procurement purpose + public-link proof
  - outreach/<date>_hosted-publishing-plan.csv # one row per site; DRAFT/pilot evidence, no automatic publication
  - outreach/<date>_wikipedia_dead_links.json  # verified research candidates; no live edit
  - outreach/<date>_press-release-plan.json    # candidate evidence + DRAFT release; never auto-submitted
...
2. **Pick tactics for this project.** Choose from the menu based on fit (you can run several):
   ...
   - **Government supplier registries** (`government-supplier-registry-profiles.md`) — screen SAM.gov, SBA Small Business Search, state vendor portals, the UK Central Digital Platform, and similar routes only when the legal entity has a real procurement purpose. Copy `assets/government-supplier-registry-opportunities.csv`. Verify official jurisdiction and eligibility rules, required declarations, privacy/publication rules, and one route-matched public profile before owner review. Registration, supplier IDs, private bid records, and share codes are not backlinks; require a live public page with the rendered destination link.
   - **Hosted-publishing/API experiments** (`hosted-publishing-experiments.md`) — screen Telegra.ph or similar services one site at a time. API availability proves that submission can be automated, not that a link is followed or a page is indexed. Copy `assets/hosted-publishing-experiment.csv`, require a unique reader job and original evidence, and draft one best-fit pilot. For Telegra.ph, preserve the source's `DA93 dofollow` language only as an unverified claim: external links on the inspected official API page rendered `nofollow`, while `index, follow` showed crawl eligibility rather than guaranteed indexation. Never create an unlimited, spun, duplicated, or exact-match-anchor batch.
   - **Press-release distribution** (`press-release-distribution.md`) — only for a real, timely company event with an owned evidence hub. First classify the actual job with `assets/press-release-distribution-registry.csv`: `openPR` for a no-cost process rehearsal; `PR Underground Basic` for a measured broad-syndication test; `EIN Presswire Basic` when the Google Business Profile map is essential; `eReleases Buzz Builder` for journalist reach. `IssueWire` stays `VERIFY_AT_CHECKOUT`; do not buy from its inconsistent public pricing cards. Recheck the exact checkout before approval. Verify destination-owned terms, a fresh public sample, link qualification, moderation, rights, disclosure, correction cost, and publication timing. Draft and hold the exact release; never repeat creator claims about followed links, outlet/backlink counts, minute-level indexing, Google News, ranking, traffic, or citations as guarantees.
...
3. **Run the guardrail before building any list.** Load `references/playbooks/authority/linkbuilding-what-not-to-do.md`. Refuse: bought cheap link packages, uniform exact-match anchor spam, forum/Reddit/comment URL spam, PBNs/link farms, off-topic or niche-mismatched links. For hosted publishing, also refuse spun/duplicated cross-site articles, doorway pages, volume-first API loops, fake authorship, and pages whose only reader value is the backlink. For press releases, also refuse low-value releases made mainly to create links, optimized-anchor distributed links, duplicate mass syndication, fake news/quotes, or unqualified paid advertorial links. If a chosen tactic drifts into these, drop it — penalties here can be permanent.

4. **Build the ranked opportunity list.** For each tactic, gather concrete targets via `ahrefs_client.py` + `serp_capture.py`: the linking page URL, referring-domain DR, dofollow?, outbound-link count (dilution), topical relevance, the tactic that fits, the contact handle/source, and a priority score. Affiliate prospects also require `competitor`, `competitor_referral_pattern`, `evidence_url`, `competitor_program_terms_verified_at`, `page_type`, `current_placement`, `ask_type`, `suggested_offer`, `offer_basis`, `vip_candidate`, and `quality_flags`. Directory/entity candidates also require source lineage, `route_type`, a destination-owned `submission_url` with `submission_url_verified_at`, `public_indexable`, `link_attribute` observed from a live listing, and `last_verified_at`; reject private-account-only or unrelated profiles. Government supplier registry rows additionally require an exact legal entity, genuine procurement-purpose evidence, jurisdiction and eligibility evidence, required attestations, an authorised reviewer, route classification, and a route-matched public profile example with the rendered website `href`/`rel`; reject SEO-only, private, buyer-only, identifier-only, ineligible, or unsupported routes. Hosted-publishing rows require the site, destination, reader job, unique thesis/evidence, disclosure, duplicate-content review, approval state, and expected referral/business metric; do not prefill a followed-link or indexed-page result. Save with `python3 scripts/report.py note --workspace <DIR> --subdir outreach --name opportunities --data <json>` → `outreach/<date>_opportunities.json`.
...
5. **DRAFT outreach (do not send).** For each high-priority target, draft a short, personal, value-first message per the relevant playbook (e.g. link-stealing: "saw you link [competitor], here's a stronger tool"; affiliate: reference the exact competing placement, verified commission structure, defensible product advantage, and only real conversion evidence; HARO: credibility hook + concrete answer; case-study trade: offer to write it). When the goal is citation coverage, offer checked facts, screenshots, criteria, methodology, and honest limitations — never fake discussion, hide sponsorship, or promise ranking/citation outcomes. Keep each message specific to the recipient. Save all drafts to `outreach/<date>_drafts.json` with `{target_url, contact, tactic, subject, body, status:"DRAFT"}`.
   ...
   **Press-release branch.** Write `outreach/<date>_press-release-plan.json` with the news contract, current destination evidence, sampled public-page evidence, rights/source ledger, exact DRAFT copy, links/anchors, required link qualification, disclosures, correction/deletion terms, costs, approval state, and post-publication verification fields. Registration, payment, submission, publication, correction, and deletion remain external actions and require explicit approval of the exact batch.

   **Hosted-publishing branch.** Copy `assets/hosted-publishing-experiment.csv` to `outreach/<date>_hosted-publishing-plan.csv`. Complete one row per candidate site, but draft only the strongest-fit pilot first. Hold account/page creation. After explicit approval of the exact article, author/disclosure, link, and destination, publish at most that pilot; then record its public URL, HTTP status, canonical, robots directive, rendered destination `href`, rendered `rel`, link visibility, dated index observations, UTM referrals, engagement, assisted conversions, article survival, and one of the registered verdicts. Do not expand merely because the API request succeeded.
...
7. **Hand off for confirmation.** Present the opportunity list + drafts to the user. Sending, posting, creating a directory/profile or hosted-publishing account, creating/editing a hosted page, submitting a listing, submitting affiliate recruitment, or uploading the disavow file happens ONLY after the user explicitly confirms — and the user (or a user-approved channel) does the actual send. For approved directory work, track `candidate -> verified -> approved -> submitted -> live | rejected | expired` in `assets/directory-submission-tracker.csv` and save the public URL/evidence. For a government supplier route, separate research approval from legal/entity attestation and exact submission approval; an authorised human reviews authentication, sensitive data, certifications, exclusions, connected persons, financial information, and declarations. Save only the non-sensitive receipt and later public-link evidence. For an approved hosted pilot, save the rendered-link and dated outcome evidence in the copied experiment ledger. Log replies/sends back into the workspace for `monitoring.md`'s leading-indicator tracking.
...
- **Government supplier route?** Require a genuine procurement purpose, official eligibility and publication evidence, a route-matched public profile example, human attestation, and exact submission approval. If supplier information is private, buyer-only, share-code-only, identifier-only, or public only after a notice/award, do not count registration as a backlink.
- **Hosted-publishing/API route?** API availability is not SEO proof. Require a unique useful article, separate site-fit row, natural link, disclosure, exact-write approval, and live rendered-page verification. Current Telegra.ph evidence does not support a blanket dofollow claim, and one pilot never authorizes an all-site batch.
...
- **Press release?** Require genuine timely news, an owned evidence page, current first-party route/terms evidence, a rights-safe DRAFT, and separate PR/referral/business measurement. A public page or followed sampled link is not permission to use press releases for ranking manipulation.
...
- `outreach/<date>_government-supplier-registries.csv` — one row per entity/registry pair with procurement-purpose, eligibility, attestation, approval, public-profile, rendered-link, and terminal-verdict evidence.
- `outreach/<date>_hosted-publishing-plan.csv` — per-site fit, exact-write approval, rendered-link, index-observation, referral, outcome, and verdict evidence; defaults to DRAFT/inconclusive.
- `outreach/<date>_disavow.txt` — optional disavow file for the user to submit.
- `outreach/<date>_press-release-plan.json` — optional evidence packet and exact DRAFT release; no submission authority.

**Done when.** A ranked opportunity list and matching personalized drafts exist in the workspace, every chosen tactic passed the guardrail, and nothing has been sent/submitted without explicit user confirmation. Directory work also has verified route evidence and a tracked public listing URL or rejection reason. Hosted-publishing work also has one row per screened site, one separately approved pilot at most, and live canonical/robots/`href`/`rel` plus outcome evidence before expansion; it never reports API success, crawl eligibility, or domain authority as a dofollow/indexing result. A press-release plan also has real-news, destination, rights, disclosure, link, correction-cost, and post-publication evidence fields, with creator claims kept separate from current observations. Wikipedia research either has ten verified candidates with dead/archive/COI evidence or an exhausted-search report; it never implies that an edit occurred. Feed approved sends + acquired-link counts into `monitoring.md` and re-check competitor backlink pace there monthly.
