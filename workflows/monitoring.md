...
outputs:
  ...
  - monitoring/<date>_agent-request-evidence.json # optional privacy-safe origin/CDN agent-request snapshot
  - audits/<date>_internal-link-architecture.json # optional contextual-link/orphan baseline
---
...
3. **Capture the rank/traffic snapshot (trailing indicators).** Pull from GSC (`gsc_pull.py`): clicks, impressions, average position, and per-keyword movement for the tracked keywords/pages. Pull from GA4 (`ga4_pull.py`): organic sessions, engagement, conversions. This is the trailing data — informative but lagging.
   ...
   **Optional agent-request layer.** When origin/CDN logs and a lawful measurement purpose are available, load `references/playbooks/maintenance/agent-readable-web-delivery.md`. Save only privacy-safe aggregate evidence to `monitoring/<date>_agent-request-evidence.json`: source, date range, normalized route, status/latency/cache class, claimed agent, published-IP verification state, documented search/training/user-triggered role, and representation. Strip secrets and personal or sensitive query data. A request does not prove parsing, indexing, training, citation, recommendation, or a business outcome.

   **Optional internal-link architecture layer.** For large sites or a material orphan/architecture concern, load `references/playbooks/maintenance/contextual-internal-link-architecture.md` and preserve `audits/<date>_internal-link-architecture.json`. Compare crawl discovery, verified orphan state, click depth, edge classes, and approved release dates with GSC/business outcomes separately. Do not treat a field-study threshold as a ranking formula or a post-release movement as causal proof.
...
- `monitoring/<date>_agent-request-evidence.json` — optional privacy-safe origin/CDN request snapshot with bot-identity and outcome limitations.
- `audits/<date>_internal-link-architecture.json` — optional contextual/template edge, orphan, concentration, and opportunity evidence.
