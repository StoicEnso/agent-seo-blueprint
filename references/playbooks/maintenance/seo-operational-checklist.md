...
| CON-05 | Publish comparison/alternative pages when justified | Live demand and SERP format support them; compared jobs overlap; criteria are explicit; product/pricing claims have sources and checked dates; material disadvantages remain visible; `free`, `open-source`, `cheaper`, and audience claims are literally true. | `content-production.md` |
...
| CON-07 | Create integration/constraint/pricing/use-case/problem/feature/audience pages where justified | Each page has distinct demand and a user job, real product truth, current proof, useful content or functionality, overlap/canonical review, and a conversion path. Consolidate token-swapped variants; avoid doorway and scaled-content abuse. | `content-production.md` |
